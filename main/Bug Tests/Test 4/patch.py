...
# [Previous copyright and imports remain the same]
...
def check_type_int(value):
    ...
    if isinstance(value, bool):
        raise TypeError(f'"{value}" is a boolean, expected integer')
    if not isinstance(value, int):
        try:
            if isinstance(value, float) and not value.is_integer():
                raise ValueError("Floats with decimal parts cannot be converted to integers")
            value = int(value)
...
def check_type_dict(value):
    ...
    if isinstance(value, str):
        value = value.strip()
        if value.startswith(('{', '[')):  # Also catches JSON arrays accidentally passed
            try:
                parsed = json.loads(value)
                if isinstance(parsed, dict):
                    return parsed
                raise TypeError('JSON string does not contain a dictionary')
            except json.JSONDecodeError as e:
                raise TypeError(f'Invalid JSON string: {str(e)}')

        # Handle key=value strings with better error messages
        try:
            return dict(item.split("=", 1) for item in re.split(r'\s*(?<!\\),\s*', value) if item)
        except ValueError as e:
            raise TypeError('Dictionary string must be in key=value format') from e

    raise TypeError(f'{type(value).__name__} cannot be converted to a dict')
...
def check_type_list(value):
    ...
    :returns: List containing the original value or split string
    """
