...
# Silence Hugging Face + logging spam
os.environ["TRANSFORMERS_NO_ADVISORY_WARNINGS"] = "true"
os.environ["TRANSFORMERS_VERBOSITY"] = "error"
os.environ["TOKENIZERS_PARALLELISM"] = "false"
logging.getLogger().setLevel(logging.ERROR)
for name in ["transformers", "sentence_transformers", "urllib3", "filelock", "datasets", "torch"]:
...
# === MODEL-SPECIFIC THRESHOLDS (from your F1 analysis) ===
model_thresholds = {
    "all-MiniLM-L6-v2": 0.05,
    "paraphrase-MiniLM-L3-v2": 0.20,
    "multi-qa-MiniLM-L6-cos-v1": 0.05,
    "sentence-t5-base": 0.80,
    "intfloat/e5-base": -1.00,
...
def compare_with_models(explanation_file, nlp_file):
    ...
    for model_name, load_model in semantic_models.items():
        try:
            ...
            # Fetch model-specific threshold
            threshold = model_thresholds.get(model_name, 0.6)  # fallback = 0.6
            verdict = "Trustworthy ✅" if score >= threshold else "Untrustworthy ❌"

            print(f"→ [{model_name}] | Score: {score:.2f} | Threshold: {threshold:.2f} | {verdict}")
